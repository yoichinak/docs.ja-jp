...
この例では、<xref:System.Xml.Linq.XContainer.Element%2A> メソッドを使用しています。このメソッドは「[方法: 単一の子要素を取得する (LINQ to XML) (C#)](how-to-retrieve-a-single-child-element-linq-to-xml.md)」というトピックで紹介されました。 また、キャストを使用して、<xref:System.Xml.Linq.XContainer.Element%2A> メソッドによって返された要素の値を取得します。  
