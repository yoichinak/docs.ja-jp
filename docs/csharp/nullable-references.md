...
この演算子について詳しくは、GitHub で [null 許容参照型のドラフト](../../_csharplang/proposals/csharp-8.0/nullable-reference-types-specification.md#the-null-forgiving-operator)仕様の提案をご覧ください。
...
- "*無関係*" : これは、C# 8 より前の状態です。 この型の変数を逆参照したり割り当てたりしても、警告は発生しません。
...
- `annotations`:null 許容注釈コンテキストは**有効**です。 null 許容警告コンテキストは**無効**です。
  - 参照型の変数は、無関係です。 null 値の許容のすべての警告は無効です。
...
- [null 許容参照のドラフト仕様](https://github.com/dotnet/csharplang/blob/master/proposals/csharp-8.0/nullable-reference-types-specification.md)
