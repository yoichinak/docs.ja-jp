...
単項 `*` 演算子は、[ポインター間接参照演算子](multiplication-operator.md#pointer-indirection-operator)です。
...
文字列連結とデリゲートの組み合わせにも、`+` 演算子が使用できます。 詳細については、「[`+` 演算子](addition-operator.md)」の記事を参照してください。
...
ユーザー定義型は、単項算術演算子 (`++`、`--`、`+`、`-`) と 2 項算術演算子 (`*`、`/`、`%`、`+`、`-`) を[オーバーロード](../keywords/operator.md)できます。 2 項演算子をオーバーロードすると、対応する複合代入演算子も暗黙的にオーバーロードされます。 ユーザー定義型は、複合代入演算子を明示的にオーバーロードすることはできません。
