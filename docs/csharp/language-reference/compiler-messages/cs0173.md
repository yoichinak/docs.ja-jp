...
CS0173 を解決するには、変換の方向またはクラスに関係なく、`class1` と `class2` の暗黙的変換が 1 つだけ存在することを確認します。 詳細については、「[暗黙的な数値変換の一覧表](../keywords/implicit-numeric-conversions-table.md)」と「[User-defined conversion operators](../operators/user-defined-conversion-operators.md)」(ユーザー定義の変換演算子) を参照してください。
