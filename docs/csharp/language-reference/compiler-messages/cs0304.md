...
```  
 ...
 この `new()` 制約によって、`T` に指定される任意の具象型が既定のパラメーターなしのコンストラクターを持つことが保証され、タイプ セーフが実現されます。 CS0304 は、型パラメーター `new` で `T` 制約を指定していないときに、クラスの本体で `T` 演算子を使用して `new()` のインスタンスを作成しようとした場合に発生します。 クライアント側のコードで、パラメーターなしのコンストラクターを持たない型を使用してジェネリック クラスをインスタンス化しようとすると、[コンパイル エラー CS0310](./cs0310.md) が生成されます。  
