...
認識できないエスケープ シーケンスです  
 ...
 予期しない文字は \\文字列[の円記号 (](../builtin-types/reference-types.md#the-string-type)) に従います。 コンパイラは、有効なエスケープ文字のうち 1 つを要求します。 詳細については、[文字のエスケープ](../../../standard/base-types/character-escapes-in-regular-expressions.md) に関するページを参照してください。  
