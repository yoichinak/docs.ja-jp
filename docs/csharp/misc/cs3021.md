...
アセンブリに CLSCompliant 属性が含まれていないため、'type' には CLSCompliant 属性が必要ありません  
 ...
 この警告は、場合に発生します。`[CLSCompliant(false)]`を true に、アセンブリ レベルの CLSCompliant 属性セットがないアセンブリ内のクラスに表示されます (行は、 `[assembly: CLSCompliant(true)]`)。 アセンブリは自らを CLS 準拠と宣言していないため、非準拠であると見なされます。そのため、アセンブリ内では非準拠を宣言する必要はありません。 CLS 準拠の詳細については、[言語の独立性および言語非依存コンポーネント](../../standard/language-independence.md)を参照してください。
