...
イベント 'event' は +=、-= の左辺にのみ使用できます。ただし、'type' 型内から使用されている場合を除きます  
 ...
 定義されているクラスの外部では、 [event](../../csharp/language-reference/keywords/event.md) は参照の追加または削除のみできます。 詳細については、「 [イベント](../../csharp/programming-guide/events/index.md)で定義されているインターフェイスのプライベート C++ 固有の実装です。  
