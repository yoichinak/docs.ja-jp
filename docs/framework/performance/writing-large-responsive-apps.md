...
### <a name="fact-3-good-tools-make-all-the-difference"></a>事実 3: 優れたツールには大きな効果がある  
 優れたツールを使用すれば、最も大きなパフォーマンスの問題 (CPU、メモリ、またはディスク) の詳細を迅速に確認し、このようなボトルネックを引き起こしているコードを特定できます。 マイクロソフトでは[、Visual Studio プロファイラ](/visualstudio/profiling/beginners-guide-to-performance-profiling)や[PerfView](https://www.microsoft.com/download/details.aspx?id=28567)などのさまざまなパフォーマンス ツールを提供しています。
...
```  
 ...
 `WriteFormattedDocComment()` の最初のバージョンでは、配列、複数の部分文字列、トリミングされた部分文字列と空の `params` 配列が割り当てられました。 また、"//"のチェックも行いました。 修正後のコードは、インデックス作成のみを使用し、割り当てを行いません。 空白以外の最初の文字を検索し、文字列が "///" で始まるかどうかを文字ごとにチェックします。 新しいコードでは`IndexOfFirstNonWhiteSpaceChar`、空白<xref:System.String.TrimStart%2A>以外の文字が出現する最初のインデックス (指定した開始インデックスの後) を返す代わりに使用されます。 この修正は完全ではありませんが、完全な解決策として類似の修正を適用する方法がわかります。 コード全体でこの方法を適用することで、`WriteFormattedDocComment()` 内のすべての割り当てを削除できます。
...
言語統合クエリ (LINQ) は、ラムダ式と組み合わせて、生産性機能の一例です。 ただし、この機能を使用すると、時間の経過に応じてパフォーマンスに大きな影響を与える可能性があり、コードを書き直す必要がある場合があります。
 ...
 **例 6 の修正**  
 ...
 完了<xref:System.Threading.Tasks.Task>した割り当てを削除するには、完了した結果を含む Task オブジェクトをキャッシュします。  
...
- [パフォーマンス プロファイリングの初心者向けガイド](/visualstudio/profiling/beginners-guide-to-performance-profiling)
- [パフォーマンス](index.md)
...
- [GitHubのドットネット/ロスリンレポ](https://github.com/dotnet/roslyn)
