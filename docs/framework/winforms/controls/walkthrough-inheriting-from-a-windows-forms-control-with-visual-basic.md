...
6. ステートメントを`Partial Public Class ValueButton`見つけて、このコントロールの継承<xref:System.Windows.Forms.UserControl>元の型をに<xref:System.Windows.Forms.Button>変更します。 `Class` これにより、継承されたコントロールが<xref:System.Windows.Forms.Button>コントロールのすべての機能を継承できるようになります。
