...
> [!NOTE]
> **ツールボックス**では、以前のバージョンの Visual Studio に含まれていた <xref:System.Windows.Forms.Splitter> コントロールが、このコントロールに置き換わっています。 `SplitContainer` コントロールは <xref:System.Windows.Forms.Splitter> コントロールより優先されます。 <xref:System.Windows.Forms.Splitter> クラスは、既存のアプリケーションの互換性のために .NET Framework に含まれていますが、新しいプロジェクトでは `SplitContainer` コントロールを使用することを強くお勧めします。  
