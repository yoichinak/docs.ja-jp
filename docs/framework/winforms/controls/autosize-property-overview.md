...
> [!NOTE]
> Visual Studio の影で、Windows フォーム デザイナー、生産性を最大化する、<xref:System.Windows.Forms.Control.AutoSize%2A>プロパティを<xref:System.Windows.Forms.Form>クラス。 デザイン時に、フォームにいなくても動作、<xref:System.Windows.Forms.Control.AutoSize%2A>プロパティに設定されて`false`その実際の設定に関係なく、します。 時に、特別な宿泊施設は行われません、および<xref:System.Windows.Forms.Control.AutoSize%2A>プロパティが適用されるプロパティの設定で指定されました。
