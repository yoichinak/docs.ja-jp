...
```  
 ...
 WCF が階層をまたいでオブジェクトを送信する場合は、値のみをシリアル化し、もう一方の階層でオブジェクトの新しいインスタンスを作成します。 オブジェクトの値とのやり取りはローカルでのみ発生し、.NET リモート処理の参照渡しのオブジェクトのような方法でもう一方の階層と通信することはありません。 詳細については、次を参照してください。[シリアル化および逆シリアル化](./feature-details/serialization-and-deserialization.md)します。  
...
5. WCF サービスは次のコードで開始できます。  
   ...
   ```  
     ...
     この ServiceHost は、開始されると web.config ファイルを使用して適切なコントラクト、バインド、エンドポイントを確立します。 構成ファイルの詳細については、次を参照してください。[構成ファイルを使用してサービスを構成する](./configuring-services-using-configuration-files.md)します。 このようなサーバーの開始方法は、自己ホストと呼ばれています。 WCF サービスをホストするための他の選択肢の詳細については、次を参照してください。[ホスティング サービス](./hosting-services.md)します。  
...
2. クライアントを使用して新しい値渡しのオブジェクト (Customer) を作成し、チャネルを作成して ICustomerService サービスと通信し、それにオブジェクトを送信します。  
 ...
 WCF での通常のデータ交換は、値渡しで行われます。 これにより、こういったデータ オブジェクトの 1 つでのメソッド呼び出しはローカルでのみ実行され、他の階層ではコードが呼び出されないことが保証されています。 返される参照渡しのオブジェクトのようなものを実現することはできますが*から*サーバーことはできません、参照渡しのオブジェクトを渡すためのクライアントの*に*サーバー。 クライアントとサーバーの間で対話を必要とするシナリオは、WCF で双方向サービスを使用することによって実現できます。 詳細については、次を参照してください。[双方向サービス](./feature-details/duplex-services.md)します。  
