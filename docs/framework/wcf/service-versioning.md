...
- アドレスの変更:たとえば、サービスは、エンドポイントが新しいアドレスを持つ別の場所に移動します。  
  
- 変更のバインド:たとえば、セキュリティメカニズムの変更やその設定の変更があります。  
  
- 実装の変更:たとえば、内部メソッドの実装が変更した場合などです。  
  
 これらの変更の中には "互換性に影響する" ものとそうでないものとがあります。 以前のバージョンで正常に処理されたすべてのメッセージが新しいバージョンで正常に処理される場合、変更は行われません。 その条件を満たしていない変更は、*互換性に影響*する変更です。  
...
### <a name="lax-versioning"></a>厳密でないバージョン管理  
 ...
 データコントラクトの厳密ではないバージョン管理の詳細なガイドラインに[ついては、「ベストプラクティス:データコントラクトの](../../../docs/framework/wcf/best-practices-data-contract-versioning.md)バージョン管理。  
...
## <a name="routing-service-and-versioning"></a>ルーティング サービスとバージョン管理  
 サービスに対して行った変更が互換性に影響するもので、複数の異なるバージョンのサービスを同時に実行する必要がある場合、WCF ルーティング サービスを使用して、メッセージを適切なサービス インスタンスにルーティングすることができます。 WCF ルーティング サービスは、メッセージの内容に基づくルーティングを使用します。つまり、メッセージ内に含まれる情報に応じて、そのメッセージのルーティング先が特定されます。 WCF ルーティングサービスの詳細については、「[ルーティングサービス](../../../docs/framework/wcf/feature-details/routing-service.md)」を参照してください。 WCF ルーティングサービスを使用してサービスをバージョン管理する方法の例[については、「方法:サービスの](../../../docs/framework/wcf/feature-details/how-to-service-versioning.md)バージョン管理。  
