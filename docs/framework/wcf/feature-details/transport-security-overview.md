...
> [!NOTE]
> SSL セキュリティを .NET Framework 3.5 以降で使用する場合、WCF クライアントは証明書ストア内の中間証明書と SSL ネゴシエーション中に受信した中間証明書の両方を使用して、サービスの証明書チェーン検証を実行します。証明 書。 .NET Framework 3.0 では、ローカルの証明書ストアにインストールされている中間証明書のみが使用されます。  
