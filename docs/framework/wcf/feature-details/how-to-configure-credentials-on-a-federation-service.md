...
1. <xref:System.ServiceModel.WSFederationHttpBinding> または同様のカスタム バインディングの構成。 適切なバインディングを作成する方法の詳細に[ついては、「方法:WSFederationHttpBinding](../../../../docs/framework/wcf/feature-details/how-to-create-a-wsfederationhttpbinding.md)を作成します。  
  
2. サービスに提示される発行済みトークンの認証方法を制御する <xref:System.ServiceModel.Security.IssuedTokenServiceCredential> の構成。  
  
 このトピックでは、2 番目の手順について詳しく説明します。 フェデレーションサービスのしくみの詳細については、「 [Federation](../../../../docs/framework/wcf/feature-details/federation.md)」を参照してください。  
...
2. CardSpace カード<xref:System.ServiceModel.Security.IssuedTokenServiceCredential.AllowUntrustedRsaIssuers%2A>などの`true`自己発行トークンを認証する場合は、プロパティをに設定します。 既定値は `false` です。  
...
1. 要素を <`serviceCredentials`> 要素の子として作成します。 `<issuedTokenAuthentication>`  
  
2. CardSpace カードなどの自己`<issuedTokenAuthentication>`発行トークン`true`を認証する場合は、要素の属性をに設定します。`allowUntrustedRsaIssuers`  
...
5. 必要に応じて、 `samlSerializer` <`issuedTokenAuthentication`の > 要素の属性をカスタム<xref:System.IdentityModel.Tokens.SamlSerializer>クラスの型名に設定します。  
