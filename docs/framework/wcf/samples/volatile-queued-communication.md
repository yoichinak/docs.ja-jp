...
> [!NOTE]
> 
  <xref:System.Messaging> を使用してキューを作成する場合、キュー名では、ローカル コンピューターを表すのにドット (.) を使用し、パスの区切りにはバックスラッシュを使用します。 Windows Communication Foundation (WCF) エンドポイントのアドレス指定、net.msmq: スキーム、ローカル マシンとそのパスにスラッシュ"localhost"を使用します。
