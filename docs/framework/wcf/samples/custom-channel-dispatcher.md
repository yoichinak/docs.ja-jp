...
## <a name="custom-servicehostbase"></a>カスタム ServiceHostBase  
 このサンプルでは、で<xref:System.ServiceModel.ServiceHostBase> <xref:System.ServiceModel.ServiceHost>はなく基本型を実装し、Windows Communication Foundation (WCF) スタックの実装をチャネルスタック上のカスタムメッセージ処理レイヤーに置き換える方法を示します。 仮想メソッド <xref:System.ServiceModel.ServiceHostBase.InitializeRuntime%2A> をオーバーライドして、チャネル リスナーとチャネル ディスパッチャーを作成します。  
