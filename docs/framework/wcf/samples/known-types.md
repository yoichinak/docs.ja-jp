...
```  
 ...
 既知の型の機能を示すために、サービスは加算および減算に`ComplexNumberWithMagnitude`のみを返すように実装されています。 (コントラクトで `ComplexNumber` を指定している場合でも、コントラクトの `KnownTypeAttribute` 属性によりこの実装は許可されます)。 乗算と除算は引き続き基本`ComplexNumber`型を返します。  
