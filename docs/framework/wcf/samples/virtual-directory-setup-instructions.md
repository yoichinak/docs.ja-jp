...
> %SystemDrive% は、インターネット インフォメーション サービス (IIS) がインストールされているドライブの場所に応じて、通常は C: または D: になります。  
 ...
 [Windows Communication Foundation サンプルの1回限りのセットアップ手順](../../../../docs/framework/wcf/samples/one-time-setup-procedure-for-the-wcf-samples.md)で、setupvroot と cleanupvroot ファイルを実行して、仮想ディレクトリを作成できます。 この仮想ディレクトリを手動で作成する場合は、次の手順に従います。  
...
1. コマンドプロンプトウィンドウを開き、「 `start inetmgr` 」と入力して、インターネットインフォメーションサービス (IIS) MMC スナップインを開きます。  
...
9. **[名前 (相対識別名)]** 列で**IIS_IUSRS**エントリを検索します。 そのエントリを選択し、 **[OK]** をクリックして [検索結果] ウィンドウを閉じます。  
