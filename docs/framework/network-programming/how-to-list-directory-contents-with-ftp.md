...
description: この記事では、FTP サーバーのディレクトリの内容を一覧表示する方法のサンプルを示します。
ms.date: 06/26/2018
dev_langs:
- csharp
- vb
...
特定のディレクトリを一覧表示する必要がある場合、<xref:System.Net.WebRequest.Create%2A?displayProperty=nameWithType> メソッドで使用している URI の終わりにそのディレクトリを追加します。
