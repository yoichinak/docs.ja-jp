...
|E_INVALIDARG|`cFunctions` 0、または`moduleIds`または`methodIds`は`NULL`します。|  
|||  
...
## <a name="remarks"></a>Remarks  
 ...
 プロファイラーは一般に、プロファイラーが 1 つ以上のメソッドをインストルメント化することを求めるユーザー入力に対応して `RequestReJIT` を呼び出します。 `RequestReJIT` 通常、作業の一部を実行して、ガベージ コレクションをトリガーする可能性のためにランタイムを中断します。 このためプロファイラーは、現在プロファイラー コールバックを実行している CLR 作成スレッドではなく、以前に作成したスレッドから `RequestReJIT` を呼び出す必要があります。  
