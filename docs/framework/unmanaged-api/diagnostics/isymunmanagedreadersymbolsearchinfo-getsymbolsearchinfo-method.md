...
## <a name="parameters"></a>パラメーター  
 `cSearchInfo`  
 から`rgpSearchInfo`のサイズを示す `ULONG32`。  
  
 `pcSearchInfo`  
 入出力検索情報を格納するために必要なバッファーのサイズを受け取る `ULONG32` へのポインター。  
  
 `rgpSearchInfo`  
 入出力返された[ISymUnmanagedSymbolSearchInfo](../../../../docs/framework/unmanaged-api/diagnostics/isymunmanagedsymbolsearchinfo-interface.md)インターフェイスに設定されたポインター。  
