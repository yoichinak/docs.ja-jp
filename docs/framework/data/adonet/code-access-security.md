...
#### <a name="connectionstring-syntax"></a>ConnectionString の構文  
 次の例では、特定の接続文字列のみ使用できるようにするために、構成ファイルの `connectionStrings` 要素を使用する方法を示しています。 参照してください[接続文字列](../../../../docs/framework/data/adonet/connection-strings.md)を格納して、構成ファイルから接続文字列の取得の詳細についてはします。  
...
#### <a name="keyrestrictions-syntax"></a>KeyRestrictions の構文  
 次の例により、同じ接続文字列を使用できるように、`Encrypt`と`Packet Size`接続文字列のいずれかのオプションが他の接続文字列オプションの使用を制限します。  
