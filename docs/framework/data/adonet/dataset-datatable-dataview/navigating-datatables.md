...
## <a name="example"></a>例  
 次の例では、 `TestConstructor`メソッドによっ<xref:System.Data.DataTable>て2つのインスタンスが作成されます。 この<xref:System.Data.DataTableReader>クラスのコンストラクターを示すために、このサンプルでは、2つの**datatable**を含む配列に基づいて新しい**DataTableReader**を作成し、単純な操作を実行して、最初の数からの内容を出力します。列をコンソールウィンドウに表示します。  
