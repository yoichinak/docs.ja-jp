...
## <a name="example"></a>例  
 <xref:System.Data.DataTable> インスタンスを作成するコンソール アプリケーションの例を次に示します。 この例では、次に、入力された <xref:System.Data.DataTable> を <xref:System.Data.DataTable.CreateDataReader%2A> メソッドを呼び出すプロシージャに渡します。このメソッドは、<xref:System.Data.DataTableReader>内に含まれる結果を反復処理します。  
