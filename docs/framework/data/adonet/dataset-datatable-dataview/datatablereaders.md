...
<xref:System.Data.DataTableReader> は、<xref:System.Data.DataTable> または <xref:System.Data.DataSet> の内容を、読み取り専用かつ前方参照専用の 1 つ以上の結果セットとして表します。  
 ...
 作成するときに、 **DataTableReader**から、 **DataTable**、結果の**DataTableReader**オブジェクトには、1 つの結果と同じデータ セットが含まれています、 **DataTable**から作成した、削除済みとしてマークされているすべての行を除く。 列の元と同じ順序で表示**DataTable**します。  
  
 A **DataTableReader**呼び出すことによって作成された場合、複数の結果セットを含めることができます<xref:System.Data.DataSet.CreateDataReader%2A>します。 結果と同じ順序では、 **Datatable**で、**データセット**オブジェクトの<xref:System.Data.DataSet.Tables%2A>コレクション。  
...
## <a name="in-this-section"></a>このセクションの内容  
 ...
 使用について説明します、**読み取り**の内容を移動する方法、 **DataTableReader**します。  
