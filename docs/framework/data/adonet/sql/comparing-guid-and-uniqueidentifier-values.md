...
### <a name="comparing-guid-values"></a>GUID 値の比較  
 `uniqueidentifier` 型の値には比較演算子が使用できます。 ただし、順序付けは、2 つの値のビット パターンの比較によっては実装されません。 `uniqueidentifier` 値に対して許可される操作は、比較 (=、< >、\<、>、\<=、> =) と、NULL (IS NULL および IS NOT NULL) のチェックのみです。 これ以外の算術演算子を使用することはできません。  
