...
物理ページングは、 [ORDER by](order-by-entity-sql.md)句の[SKIP](skip-entity-sql.md)サブ句および[LIMIT](limit-entity-sql.md)サブ句を使用して実行できます。 物理的ページングを決定的に実行するには、SKIP と LIMIT を使用する必要があります。 結果内の行の数を非決定的な方法で制限する場合は、 [TOP](top-entity-sql.md)を使用する必要があります。 TOP および SKIP/LIMIT は、同時には指定できません。  
