...
ms.locfileid: "70249353"
---
# <a name="query-expression-syntax-examples-partitioning"></a>クエリ式の構文例:パーティション分割
このトピックの例では、クエリ式の<xref:System.Linq.Enumerable.Skip%2A>構文<xref:System.Linq.Enumerable.Take%2A>を使用して、メソッドとメソッドを使用して、 [AdventureWorks Sales Model](https://archive.codeplex.com/?p=msftdbprodsamples)に対してクエリを実行する方法を示します。 これらの例で使用されている、AdventureWorks Sales Model は、AdventureWorks サンプル データベースの Contact、Address、Product、SalesOrderHeader、SalesOrderDetail の各テーブルから作成されています。  
