...
## <a name="datetime"></a>DateTime  
 ...
 日付部分の形式は次のとおりでなければなりません: `YYYY` @ no__t @ no__t @ no__t @。 @no__t ここで、-5 は 0001 ~ 9999 の4桁の年の値、`MM` は1から12の月、`DD` は、指定された月 @no__t に有効な日の値です。  
...
## <a name="guid"></a>GUID  
 `GUID` リテラルは、グローバル一意識別子を表します。 これは、キーワード `GUID` の後に、1つの引用符で囲まれた*レジストリ*8-4-4-4-12 形式の16進数の数字で構成されるシーケンスです。 16 進数字の大文字と小文字は区別されません。  
