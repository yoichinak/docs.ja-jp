...
## <a name="datetime"></a>DateTime  
 ...
 日付部分の形式は、`YYYY`-`MM`-`DD`である必要があります。ここで、`YYYY` は 0001 ~ 9999 の4桁の年の値、`MM` は1から12の間の月、`DD` は特定の月の `MM`に対して有効な日の値です。  
...
## <a name="guid"></a>Guid  
 `GUID` リテラルは、グローバル一意識別子を表します。 これは、キーワード `GUID` によって作成されたシーケンスであり、その後に、単一引用符で囲まれた*レジストリ*形式: 8-4-4-4-12 という形式の16進数字が続きます。 16 進数字の大文字と小文字は区別されません。  
