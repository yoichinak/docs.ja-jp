...
## <a name="datetime"></a>DateTime  
 ...
 日付部分は、の形式にする`YYYY`必要が`YYYY`あります。は、0001 ~ 9999 `DD` - `MM`の`DD` 4 桁の年の値、は 1 ~ 12 の月、は次のようになります。 `MM` -指定された月`MM`に有効な日付の値。  
...
## <a name="guid"></a>Guid  
 `GUID` リテラルは、グローバル一意識別子を表します。 これは、キーワード`GUID`で構成されるシーケンスであり、その後に*レジストリ*形式と呼ばれる16進数の数字が続きます。8-4-4-4-12 は単一引用符で囲まれています。 16 進数字の大文字と小文字は区別されません。  
