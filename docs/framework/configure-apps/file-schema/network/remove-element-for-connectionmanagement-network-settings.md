...
---
# <a name="remove-element-for-connectionmanagement-network-settings"></a>\<connectionManagement の > 要素を削除する (ネットワーク設定)
...
## <a name="remarks"></a>Remarks  
 要素`remove`は、指定されたサーバーの接続管理リストのエントリを削除します。  
...
## <a name="example"></a>例  
 次の例では、サーバー `www.adventure-works.com`に対する接続管理の一覧のエントリをすべて削除し、サーバー `www.contoso.com`への接続を4つ、他のすべてのサーバーへの接続を2つ使用するようにアプリケーションを構成します。  
