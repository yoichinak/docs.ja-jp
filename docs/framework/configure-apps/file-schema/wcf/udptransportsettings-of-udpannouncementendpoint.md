...
この構成要素は、 [ \<udptransportsettings >](udpannouncementendpoint.md)の UDP トランスポート設定を公開します。  
...
|要素|説明|  
|-------------|-----------------|  
|[\<Udptransportsettings >](udpannouncementendpoint.md)|固定アナウンス コントラクトと UDP トランスポート バインディングを持つ標準エンドポイント。|  
