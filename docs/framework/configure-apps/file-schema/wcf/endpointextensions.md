...
```  
 ...
 標準エンドポイントを追加すると、次の例に示すように、このエンドポイントを使用できます。 `kind` `<endpointExtensions>` [エンドポイント > 要素で、属性は、セクションに登録されている標準エンドポイントの種類を指定します。 \<](endpoint-element.md) 属性は、 `<standardEndpoints>`セクション内の標準`name`エンドポイントの構成要素の属性と同じになります。 `endpointConfiguration`  
