...
2. トレース スイッチを構成します。 詳しくは、[トレース・スイッチの構成を](#configure)参照してください。  
...
## <a name="configuring-trace-switches"></a>トレース スイッチの構成  
 ...
 スイッチは、.config ファイルを使用して構成されています。 Web アプリケーションでは、これは、プロジェクトに関連付けられた Web.config ファイルです。 Windows アプリケーションでは、このファイルの名前 (アプリケーション名). exe.config.配置されたアプリケーションでは、このファイルは実行可能ファイルと同じフォルダーに存在する必要があります。  
...
2. プロジェクトに構成ファイル (app.config または Web.config) が含まれない場合は、**[プロジェクト]** メニューから **[新しい項目の追加]** を選択します。  
    ...
    - **Visual C# :****[新しい項目の追加]** ダイアログ ボックスで **[XML ファイル]** を選択します。 このファイルに**名前を付けます。** XML エディタで、XML 宣言の後に、次の XML を追加します。  
