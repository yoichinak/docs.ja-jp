...
- インスタンスが初めて永続化される前にインスタンスで未処理の例外が発生した場合。 次の状況が発生します。  
    ...
    - 場合の値、 **UnhandledExceptionAction**プロパティに設定されて**AbandonAndSuspsend**にインスタンスの状態が設定されている、サービスの展開情報が永続性データベースに書き込まれます**中断**します。 インスタンスを再開、取り消し、または終了することはできません。 インスタンスはまだ永続化されておらず、インスタンスのデータベース エントリが完成していないため、サービス ホストはインスタンスを読み込むことができません。  
