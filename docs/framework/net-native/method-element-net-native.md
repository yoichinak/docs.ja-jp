...
|Value|説明|  
|-----------|-----------------|  
|*method_name*|メソッド名。 メソッドの型は、親[\<の型>](type-element-net-native.md)または[\<TypeInstantiation>](typeinstantiation-element-net-native.md)要素によって定義されます。|  
...
|要素|説明|  
|-------------|-----------------|  
|[\<パラメータ>](parameter-element-net-native.md)|メソッドに渡された引数の型にポリシーを適用します。|  
|[\<ジェネリックパラメーター>](genericparameter-element-net-native.md)|ジェネリック型またはメソッドのパラメーターの型にポリシーを適用します。|  
|[\<暗黙的な型>](impliestype-element-net-native.md)|型にポリシーを適用します (含んでいる `<Method>` 要素によって表されるメソッドにそのポリシーが適用されている場合)。|  
|[\<型パラメーター>](typeparameter-element-net-native.md)|メソッドに渡された <xref:System.Type> 引数によって表される型にポリシーを適用します。|  
  
### <a name="parent-elements"></a>親要素  
  
|要素|説明|  
|-------------|-----------------|  
|[\<タイプ>](type-element-net-native.md)|型とそのすべてのメンバーにリフレクション ポリシーを適用します。|  
|[\<入力インスタンス化>](typeinstantiation-element-net-native.md)|構築されたジェネリック型とそのすべてのメンバーにリフレクション ポリシーを適用します。|  
...
## <a name="remarks"></a>解説  
 ...
 `<Method>` 要素を使用してコンストラクターのランタイム リフレクション ポリシーを定義することはできません。 代わりに、[\<アセンブリ>](assembly-element-net-native.md)、[\<名前空間>](namespace-element-net-native.md)、[\<型>、](type-element-net-native.md)または[\<TypeInstantiation>](typeinstantiation-element-net-native.md)要素の`Activate`属性を使用します。  
...
## <a name="example"></a>例  
 ...
 これらの例外はすべて[\<、Type>](type-element-net-native.md)要素を使用してメタデータが存在する必要がある型を定義し、動的に呼び出`<Method>`すことができるメソッド オーバーロードの実装も確実に存在するように要素を追加することで排除できます。 これらの例外を排除し、例をエラーなしで実行できるようにした default.rd.xml ファイルを次に示します。  
...
- [\<メソッドインスタンス化>要素](methodinstantiation-element-net-native.md)
