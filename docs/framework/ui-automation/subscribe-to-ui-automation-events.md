...
## <a name="example"></a>例  
 次のコード例では、ボタンなどのコントロールが呼び出された場合に生成されるイベントに対してイベント ハンドラーを登録し、アプリケーション フォームが閉じられた時にそのイベント ハンドラーを削除します。 イベントは、パラメーターとして <xref:System.Windows.Automation.AutomationEvent> に渡される <xref:System.Windows.Automation.Automation.AddAutomationEventHandler%2A> によって識別されます。  
