...
### <a name="reasons-for-using-formatted-text"></a>書式設定されたテキストを使用する理由  
 ...
 また、<xref:System.Windows.Media.FormattedText> オブジェクトは、テキスト指向の <xref:System.Windows.Media.DrawingVisual>派生オブジェクトを作成する場合に便利です。 <xref:System.Windows.Media.DrawingVisual> は、図形、画像、またはテキストを表示するために使用される軽量の描画クラスです。 詳細については、「[DrawingVisuals を使用したヒット テストのサンプル](https://go.microsoft.com/fwlink/?LinkID=159994)」を参照してください。  
...
|DT_EDITCONTROL|[なし]|任意 スペースの幅および最後の行のレンダリングは、フレームワークのエディット コントロールと同じです。|  
...
|DT_EXPAND_TABS|[なし]|任意 タブは自動的に全角 4 文字分ごとに展開されます。これは、言語非依存文字の約 8 文字分の幅です。|  
|DT_EXTERNALLEADING|[なし]|任意 外部レディングは、常に行間隔に含まれます。 ユーザー定義の行間を作成するには、<xref:System.Windows.Media.FormattedText.LineHeight%2A> プロパティを使用します。|  
...
|DT_NOPREFIX|[なし]|任意 文字列内の '&' 文字は、常に通常の文字として扱われます。|  
...
|DT_SINGLELINE|[なし]|任意 <xref:System.Windows.Media.FormattedText> オブジェクトは、<xref:System.Windows.Media.FormattedText.MaxTextWidth%2A> プロパティが設定されているか、テキストにキャリッジリターン/ラインフィード (CR/LF) が含まれている場合を除き、単一行コントロールとして動作します。|  
|DT_TABSTOP|[なし]|ユーザー定義のタブ位置はサポートされていません。|  
|DT_TOP|<xref:System.Windows.Media.FormattedText.Height%2A>|任意 上端揃えが既定値です。 その他の垂直方向の配置値は、<xref:System.Windows.Media.FormattedText.Height%2A> プロパティを使用して、適切な Win32 DrawText ' y ' 位置を計算することによって定義できます。|  
...
|DT_WORDBREAK|[なし]|任意 単語分割は、<xref:System.Windows.Media.FormattedText> オブジェクトで自動的に行われます。 無効にすることはできません。|  
