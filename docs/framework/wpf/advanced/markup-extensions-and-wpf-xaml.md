...
## <a name="extension-classes"></a>\*拡張クラス  
 ...
 たとえば、<xref:System.Windows.StaticResourceExtension> クラスは、実際のリソース検索の表面実装を提供します。これにより、<xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A> の実装は、要求されたオブジェクトを返します。その特定の実装の入力は、`x:Key`別のリソース。 既存のマークアップ拡張を使用している場合は、この実装の詳細の大部分は重要ではありません。  
...
- マークアップ拡張でコンストラクターの動作とプロパティの設定の動作の結果が類似している場合は、どちらの動作を使用しても問題はありません。 複数の設定可能なプロパティを持つマークアップ拡張では、"*プロパティ*`=`*値*" のペアの方がよく使用されます。単に、その方がマークアップの意図が明確になり、コンストラクター パラメーターを間違って入れ替える可能性も低いためです。 (プロパティ = 値のペアを指定する場合、これらのプロパティは任意の順序で指定できます)。また、マークアップ拡張機能が、設定可能なすべてのプロパティを設定するコンストラクターパラメーターを提供するという保証はありません。 たとえば、<xref:System.Windows.Data.Binding> はマークアップ拡張*機能であり、プロパティ`=`* *値*フォームの拡張機能を通じて設定できる多くのプロパティがありますが、<xref:System.Windows.Data.Binding> では、パラメーターなしのコンストラクターと、初期パス。  
