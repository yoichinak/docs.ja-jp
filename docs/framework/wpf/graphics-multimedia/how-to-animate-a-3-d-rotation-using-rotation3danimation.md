...
次の例は、3D オブジェクトに適用された<xref:System.Windows.Media.Animation.Rotation3DAnimation><xref:System.Windows.Media.Media3D.RotateTransform3D.Rotation%2A><xref:System.Windows.Media.Media3D.RotateTransform3D>オブジェクトのプロパティをアニメーション化することによって、3D オブジェクトを「ぐらつく」ときに回転させる方法を示しています。  
