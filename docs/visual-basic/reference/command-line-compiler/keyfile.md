...
## <a name="example"></a>例  
 次のコードでは、ソースファイル `Input.vb` でコンパイルし、キーファイルを指定しています。  
