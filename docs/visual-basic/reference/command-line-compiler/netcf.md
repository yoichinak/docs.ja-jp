...
'-netcf' オプションは、 [-sdkpath](../../../visual-basic/reference/command-line-compiler/sdkpath.md)と共に使用するように設計されています。 '-netcf' によって無効にされた言語機能は、`-sdkpath` を対象とするファイルには存在しないものと同じ言語機能です。
...
- すべての形式の遅延バインディングは無効になっています。 コンパイル時のエラーは、認識された遅延バインディングのシナリオが検出された場合に生成されます。 次のプログラムは '-netcf' を指定せずにコンパイルして実行しますが、コンパイル時に `-netcf` で失敗します。
