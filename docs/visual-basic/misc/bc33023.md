...
比較演算子または論理演算子が戻り値の型以外の宣言、[ブール データ型](../../visual-basic/language-reference/data-types/boolean-data-type.md)します。  
 ...
 論理演算子 (`And`、 `AndAlso`、 `Not`、 `Or`、 `OrElse`、 `Xor`) は、ブール値のドメイン内でのみ機能します。 詳細については、[論理および Visual Basic でビットごとの演算子](../../visual-basic/programming-guide/language-features/operators-and-expressions/logical-and-bitwise-operators.md)を参照してください。  
