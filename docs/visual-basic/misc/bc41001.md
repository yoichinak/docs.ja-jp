---
title: "クラス '<classname>' は、基本クラス '<baseclassname>' にある'<constructorname>' が古い形式に設定されているため、'Sub New'を宣言しなければなりません。"
ms.date: 07/20/2015
f1_keywords:
- bc41001
- vbc41001
...
---
# <a name="class-classname-should-declare-a-sub-new-because-the-constructorname-in-its-base-class-baseclassname-is-marked-obsolete"></a>クラス '\<classname>' は、基本クラス '\<baseclassname>' にある'\<constructorname>' が古い形式に設定されているため、'Sub New'を宣言しなければなりません。
クラス宣言にコンストラクターが含まれず、基底クラスのコンストラクターが <xref:System.ObsoleteAttribute> 属性および警告として扱うことを示すディレクティブでマークされています。  
 ...
 既定では、 <xref:System.ObsoleteAttribute.IsError%2A> の <xref:System.ObsoleteAttribute> プロパティが `False`であるため、メッセージは警告となります。 警告を非表示にする方法や、警告をエラーとして扱う方法の詳細については、「[Visual Basic での警告の構成](/visualstudio/ide/configuring-warnings-in-visual-basic)」をご覧ください。  
