...
---
# <a name="isnot-requires-operands-that-have-reference-types-but-this-operand-has-the-value-type-typename"></a>' IsNot ' には参照型を持つオペランドが必要ですが、このオペランドの\<値型は ' typename > ' です。
式で、値型のオペランドを少なくとも 1 つ持つ [IsNot 演算子](../../visual-basic/language-reference/operators/isnot-operator.md) が使用されています。  
