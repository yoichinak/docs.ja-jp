...
---
# <a name="event-eventname-implicitly-declares-membername-which-conflicts-with-a-member-in-the-base-type-classname-and-so-the-event-should-be-declared-shadows"></a>イベント '\<eventname>' は、ベース \<type> '\<classname>' のメンバと競合する '\<membername>' を暗黙的に宣言するため、このイベントは'Shadows' と宣言しなければなりません
