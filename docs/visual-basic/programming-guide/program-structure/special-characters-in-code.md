...
## <a name="parentheses"></a>かっこ
 `Sub` や `Function` などのプロシージャを定義するときは、かっこを使用します。 すべてのプロシージャ引数リストをかっこで囲む必要があります。 また、変数や引数を論理的なグループにまとめるためにかっこを使用します。特に、複雑な式において演算子の優先順位の既定の順序をオーバーライドします。 次に例を示します。

 [!code-vb[VbVbcnConventions#11](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbcnConventions/VB/Class1.vb#11)]

 前述のコードを実行すると、 `d` の値は8.225 で、 `e` の値は3です。  `d` の計算では、 `+` より `/` が上という既定の優先順位が使用されます。これは `d = b + (c / a)` と同じです。 `e` の計算のかっこは、既定の優先順位をオーバーライドします。
