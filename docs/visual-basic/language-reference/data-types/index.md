...
- **オーバーヘッド。** 複合型によっては、追加のメモリ要件があります。 たとえば、配列は配列自体に追加のメモリを使用し、次元ごとにも使用します。 32ビットプラットフォームでは、このオーバーヘッドは現在12バイト、各ディメンションに8バイトを加えたものです。 64ビットプラットフォームでは、この要件は2倍になります。  
