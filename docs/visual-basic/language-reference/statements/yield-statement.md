...
## <a name="exception-handling"></a>例外処理  
 `Yield` ステートメントは、Try の `Try` ブロック内にある場合があります[Try...Catch...Finally ステートメント](../../../visual-basic/language-reference/statements/try-catch-finally-statement.md)。 `Yield` ステートメントを持つ `Try` ブロックは `Catch` ブロックを持つことができ、`Finally` ブロックを持つことができます。  
