...
|`datatype`|[@No__t-1](option-infer-statement.md)がオン (既定値) または @no__t が既に宣言されている場合は省略可能です。`Option Infer` がオフで `element` がまだ宣言されていない場合は必須です。 `element` のデータ型です。|
|`group`|必須。 コレクション型またはオブジェクトである型を持つ変数。 を繰り返すコレクション`statements`を参照します。|
|`statements`|任意。 内の`For Each` `Next` 各項目に対して実行される、との間の1つ`group`以上のステートメント。|
|`Continue For`|任意。 `For Each`ループの開始位置に制御を転送します。|
|`Exit For`|任意。 制御を`For Each`ループの外に転送します。|
...
次の例は、入れ子になった `For Each`... `Next` を示しています。 構成.
...
[Exit For](../../../visual-basic/language-reference/statements/exit-statement.md)ステートメントを実行すると、`For`... @no__t を終了します。 ループし、 `Next`ステートメントの後のステートメントに制御を転送します。
...
- @No__t-0... `Catch`... `Finally` で例外がキャッチされました。`Finally`ブロックの最後に`Exit For`を使用できます。
...
反復子を呼び出すには、 `For Each...Next`ステートメントを使用します。 `For Each` ループの繰り返しごとに、反復子を呼び出します。 反復子で`Yield` ステートメントに到達すると、ステートメント内の式が返され、コード内の現在の位置が保持`Yield`されます。 次回、反復子が呼び出されると、この位置から実行が再開されます。
...
ランタイムは、の`group`要素をに`element`変換できる必要があります。 [@No__t-0] ステートメントは、拡大変換と縮小変換の両方を許可するかどうかを制御します (`Option Strict` はオフ、既定値)。または、拡大変換のみが許可されるかどうかを制御します (`Option Strict` がオン)。 詳細については、「[縮小変換](#narrowing-conversions)」を参照してください。
