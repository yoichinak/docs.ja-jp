...
- [Boolean](../../../visual-basic/language-reference/data-types/boolean-data-type.md): 2種類の値
...
## <a name="relational-and-bitwise-comparisons"></a>関係とビットごとの比較  
 リレーショナル操作 (`=`、`<>`、`<`、`>`、`<=`、`>=`) の結果のデータ型は、常に[Boolean データ型](../../../visual-basic/language-reference/data-types/boolean-data-type.md)`Boolean`です。 これは、`OrElse`オペランドの論理演算 (`And`、`AndAlso`、`Not`、`Or`、`Xor`、`Boolean`) にも当てはまります。  
