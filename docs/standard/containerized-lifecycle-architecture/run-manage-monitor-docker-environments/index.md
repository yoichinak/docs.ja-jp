...
(インフラストラクチャ アーキテクチャとプラットフォーム テクノロジ) を運用環境でコンテナー化されたアプリケーションを実行する方法は非常に関連し、選択したこの電子書籍の第 1 章で説明したアーキテクチャと開発プラットフォームにします。

この章は、特定の製品およびマイクロソフトのテクノロジを調査し、効果的に拡張性が高く、実行に使用できるベンダーの他の HA 分散アプリケーション、および管理し、IT の視点から監視する方法。
