...
> [!NOTE]
> このトピックでは<xref:System.TimeZone> 、その機能がほぼ完全にクラスに<xref:System.TimeZoneInfo>組み込まれているため、説明しません。 可能な限り、クラス<xref:System.TimeZoneInfo> <xref:System.TimeZone>の代わりにクラスを使用します。
