---
title: データの暗号化
description: .NET でのデータの暗号化について説明します。 ストリームで対称暗号化を使用することも、少数のバイトで非対称暗号化を使用することもできます。
ms.date: 03/30/2017
