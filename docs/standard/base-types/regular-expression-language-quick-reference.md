...
|文字クラス|説明|パターン|一致件数|  
|---------------------|-----------------|-------------|-------------|  
|`[` *character_group* `]`|*character_group*内の任意の 1 文字と一致します。 既定では、大文字と小文字が区別されます。|`[ae]`|『`"a"`』の「`"gray"`」<br /><br /> 『`"lane"`』の「`"a"`」、「`"e"`」|  
...
|`.`|ワイルドカード:\n を除く任意の 1 文字と一致します。<br /><br /> リテラルのピリオド文字 (. または `\u002E`) と一致させるには、この文字の前にエスケープ文字 (`\.`) を指定します。|`a.e`|『`"ave"`』の「`"nave"`」<br /><br /> 『`"ate"`』の「`"water"`」|  
...
|`\s`|空白文字と一致します。|`\w\s`|『`"D "`』の「`"ID A1.3"`」|  
|`\S`|空白以外の文字と一致します。|`\s\S`|『`" _"`』の「`"int __ctr"`」|  
|`\d`|10 進数字と一致します。|`\d`|『`"4"`』の「`"4 = IV"`」|  
...
|アサーション|説明|パターン|一致件数|  
|---------------|-----------------|-------------|-------------|  
|`^`|既定では、文字列の先頭で一致する必要があります。複数行モードでは、行の先頭で一致する必要があります。|`^\d{3}`|『`"901"`』の「`"901-333-"`」|  
|`$`|既定では、文字列の末尾で一致するか、文字列の末尾にある `\n` の前で一致する必要があります。複数行モードでは、行の末尾の前で一致するか、行の末尾にある `\n` の前で一致する必要があります。|`-\d{3}$`|『`"-333"`』の「`"-901-333"`」|  
|`\A`|文字列の先頭で一致する必要があります。|`\A\d{3}`|『`"901"`』の「`"901-333-"`」|  
|`\Z`|文字列の末尾で一致するか、文字列の末尾にある `\n` の前で一致する必要があります。|`-\d{3}\Z`|『`"-333"`』の「`"-901-333"`」|  
|`\z`|文字列の末尾で一致する必要があります。|`-\d{3}\z`|『`"-333"`』の「`"-901-333"`」|  
...
|------------------------|-----------------|-------------|-------------|  
|`(` *subexpression* `)`|一致した部分式をキャプチャして、1 から始まる序数を代入します。|`(\w)\1`|『`"ee"`』の「`"deep"`」|  
|`(?<` *name* `>` *subexpression* `)`|一致した部分式を名前付きグループにキャプチャします。|`(?<double>\w)\k<double>`|『`"ee"`』の「`"deep"`」|  
|`(?<` *name1* `-` *name2* `>` *subexpression* `)`|グループ定義の均等化を定義します。 詳細については、「 [グループ化構成体](grouping-constructs-in-regular-expressions.md)」の「グループ定義の均等化」を参照してください。|`(((?'Open'\()[^\(\)]*)+((?'Close-Open'\))[^\(\)]*)+)*(?(Open)(?!))$`|『`"((1-3)*(3-1))"`』の「`"3+2^((1-3)*(3-1))"`」|  
|`(?:` *subexpression* `)`|非キャプチャ グループを定義します。|`Write(?:Line)?`|『`"WriteLine"`』の「`"Console.WriteLine()"`」<br /><br /> 『`"Write"`』の「`"Console.Write(value)"`」|  
...
|`{` *n* `,` *m* `}`|直前の要素と *n* 回以上 *m* 回以下で一致します。|`"\d{3,5}"`|`"166"`、 `"17668"`<br /><br /> 『`"19302"`』の「`"193024"`」|  
...
|-----------------------------|-----------------|-------------|-------------|  
|`\` *number*|前方参照。 番号付き部分式の値に一致します。|`(\w)\1`|『`"ee"`』の「`"seek"`」|  
|`\k<` *name* `>`|名前付き前方参照。 名前付きの式の値に一致します。|`(?<char>\w)\k<char>`|『`"ee"`』の「`"seek"`」|  
...
|``$` ``|一致の前にある入力文字列のすべてのテキストに置換されます。|`B+`|``$` ``|`"AABBCC"`|`"AAAACC"`|  
