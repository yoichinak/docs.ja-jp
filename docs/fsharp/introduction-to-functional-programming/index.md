...
# <a name="introduction-to-functional-programming-in-f">F# での関数型プログラミングの概要</a>
...
* **関数** - 関数とは、指定した入力から出力を生成する構成要素です。正式には、アイテムを 1 つのセットから別のセットに_マップ_するものです。この形式は、特にデータのコレクションで動作する関数を使用する場合に、多くの方法で具体的に持ち込まれます。これは、関数型プログラミングで最も基本的な (かつ重要な) 概念です。
...
* **参照透過性** - 参照透過性とは、プログラムの動作に影響を与えずにその出力を置き換えられる式のプロパティです。
