...
これは、関数の型シグネチャと呼ばれるものです。  次のように読み取ることができます。「square は、x という名前の整数を受け取り、整数を生成する関数」と読むことができます。  コンパイラがこの時点では`square` に`int` 型を与えていることに注意してください。これは乗算が *あらゆる* 型に一般的なのではなく、ある閉じた型の集合に一般的であるためです  F# コンパイラはこの時点で`int` を選択しましたが、例えば `float` といった別の型で `square` を呼び出すと、型シグネチャを調整します。
