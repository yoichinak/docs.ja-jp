...
多くの場合、バックグラウンド作業には複数のスレッドの使用が含まれますが、非同期性とマルチスレッドの概念を個別に考慮することが重要です。 実際、別々の検討事項であり、互いに他社を意味する訳ではありません。 この記事では、これについてさらに詳しく説明します。
...
もう 1 つの重要な行は `Async.RunSynchronously` の呼び出しです。 これは、 F#の非同期計算を実際に実行する際に呼び出す必要のある Asyncモジュール開始関数の1つです。
...
2. F# での非同期プログラミングは、マルチスレッドを抽象的したものではありません。
