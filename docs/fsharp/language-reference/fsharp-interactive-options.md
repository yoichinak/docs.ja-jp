...
|**--lib:&lt;フォルダー一覧&gt;**<br /><br />**-I: フォルダー一覧の&lt;&gt;**|**Fsc.exe**コンパイラオプションと同じです。 詳細については、「[コンパイラ オプション](compiler-options.md)」を参照してください。|
...
|**--nowarn:&lt;警告-リスト&gt;**|**Fsc.exe**コンパイラオプションと同じです。 詳細については、「[コンパイラ オプション](compiler-options.md)」を参照してください。|
...
|**--simpleresolution すべての方法**|MSBuild の解決ではなく、ディレクトリベースのルールを使用してアセンブリ参照を解決します。|
|**--tail**[ **+** &#124; **-** ]|tail IL 命令の使用を有効または無効にします。有効にすると、スタック フレームが tail 再帰関数で再利用されます。 既定では、このオプションは有効になっています。|
|**--targetprofile:&lt;文字列&gt;**|このアセンブリのターゲットフレームワークプロファイルを指定します。 有効な値は、mscorlib、netcore、または netcore です。  既定値は mscorlib です。|
