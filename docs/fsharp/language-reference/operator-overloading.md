...
標準演算子はすべてオーバーロードすることができますが、特定の文字を並べて新しい演算子を作成することもできます。 使用できる演算子文字`!`は`%` `&`、 `*` 、、`>`、、、、、、、、、です。 `+` `-` `.` `/` `<` `=` `?`、 、`@` 、`|`、および。`~` `^` `~` には、演算子を単項演算子にするという特別な意味があるため、演算子の文字シーケンスに含めることはできません。 すべての演算子を単項にすることはできません。
